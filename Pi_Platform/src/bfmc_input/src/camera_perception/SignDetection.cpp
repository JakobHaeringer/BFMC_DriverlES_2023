/**
 * @file SignDetection.cpp
 * @author Julius Baechle, Kadir Oezer, Noah Koehler
 * @brief This file implements the sign detection algorithm used in the BFMC 2023.
 * @version 1.0
 * @date 2023-06-15
 */

#include "camera_perception/SignDetection.hpp"
#include "camera_perception/AverageDistanceImage.hpp"
#include <math.h>
...
SignDetection::SignDetection(std::string engine_path) {
    m_detector.init();
 ...
 */
std::vector<Detection> SignDetection::detect(cv::Mat color_img, cv::Mat depth_img) {
...
std::vector<Detection> SignDetection::mapDetections(std::vector<yolov5::Detection> yolov5Detections) {
 ...
 */
std::vector<Detection> SignDetection::evaluateDistance(std::vector<Detection> detections, cv::Mat depth_img) {
    for (Detection& d : detections) {
#ifdef ITMOVES_SIMULATOR
        // The depth value is encoded in millimeters, so convert it to meters.
        d.distance = depth_img.at<uint16_t>(d.y, d.x) / 1000.0f;
#else
 ...
 * class type.
 */
std::vector<Detection> SignDetection::evaluateSide(std::vector<Detection> detections, int width) {
    for (Detection& d : detections)
 ...
 */
std::vector<Detection> SignDetection::filterMisdetections(std::vector<Detection> detections) {
